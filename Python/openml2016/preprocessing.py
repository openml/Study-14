...
[1, 0, 2]])  # doctest: +ELLIPSIS
    ...
    def _fit_transform(self, X):
        ...
        # First increment everything by two to account for the fact that
        # np.NaN will get an index of one, index of zero is not assigned to
        # also work with sparse data
        if sparse.issparse(X):
            X.data += 2
            X.data[~np.isfinite(X.data)] = 1
        else:
            X += 2
            X[~np.isfinite(X)] = 1
        ...
        if sparse.issparse(X):
            n_values = X.max(axis=0).toarray().flatten() + 1
        else:
    ...
    def _transform(self, X):
        ...
        # Replace all indicators which are out of bounds by 'NaN' (index 1)
...
class ItemSelector(BaseEstimator, TransformerMixin):
    """Select columns (features) based on their index.

    Notes
    -----
    This is a copy of from the scikit-learn example "Feature Union with
    Heterogeneous Data Sources". This code is licensed under a BSD license.

    Parameters
    ----------
    indices : list or array-like
        An array which can be used for numpy fancy indexing on the columns of
        the feature matrix.
    """
    ...
    def transform(self, X):
        if self.indices is None:
            indices = np.array([False] * X.shape[1])
        else:
